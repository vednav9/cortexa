...
import { FiSend, FiFile, FiLoader, FiZap } from "react-icons/fi";
...
const inferSourceType = (src = {}) => {
    if (src?.type) return String(src.type).toLowerCase();
    return src?.url ? "web" : "document";
};

const getSourceWebsite = (src = {}) => {
    if (src?.source_site) return String(src.source_site);
    if (!src?.url) return "";
    try {
        return new URL(src.url).hostname.replace(/^www\./i, "");
    } catch {
        return "";
    }
};

const normalizeSourcesForDisplay = (sources = []) => {
...
const formatSearchMethod = (method = "rag") =>
    String(method || "rag")
        .replace(/\+/g, " + ")
        .toUpperCase();
...
const Message = ({ msg, brand, rgb }) => {
    ...
    const orderedSources = normalizeSourcesForDisplay(msg.sources || []);
    const hasWebSources = orderedSources.some((src) => src.type === "web");
    const shouldShowWebBadge =
        Boolean(msg.usedWebSearch) ||
        String(msg.searchMethod || "").includes("web") ||
        hasWebSources;

    const webCitationSites = Array.from(
        new Set(
            orderedSources
                .filter((src) => src.type === "web")
                .map((src) => src.source_site)
                .filter(Boolean),
        ),
    ).slice(0, 3);
    ...
    return (
        ...
        >
            ...
            >
                {/* Bubble */}
                <div
                    className={`px-4 py-3 rounded-2xl text-[13.5px] leading-[1.65] whitespace-pre-wrap ${
                        isUser
                ...
                >
                    ...
                    {/* Sources */}
                    {/* Replace the Sources section inside Message component */}
                    {orderedSources.length > 0 && (
                        <div className="mt-2.5 pt-2.5 border-t border-gray-100 space-y-1">
                            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">
                                ...
                                <span
                                    style={{
                                        color: shouldShowWebBadge
                                            ? "#f59e0b"
                                            : undefined,
                                    }}
                                >
                                    {formatSearchMethod(msg.searchMethod)}
                                </span>
                            </p>
                            {orderedSources.slice(0, 6).map(
                                (
                                    src,
                                    i,
                                ) => (
                                        ...
                                        {src.type === "document" ? (
                                            ...
                                            <>
                                                ...
                                                <a
                                                    ...
                                                    style={{ color: brand }}
                                                    title={src.url} // show full URL on hover
                                                >
                                                    {src.source_site ||
                                                        src.document_name ||
                                                        src.title ||
                            ...
                            {shouldShowWebBadge && (
                                <p className="text-[10px] text-amber-500 mt-1 flex items-center gap-1">
                                    ...
                                    Results supplemented from web search
                                    {webCitationSites.length > 0
                                        ? `: ${webCitationSites.join(", ")}`
                                        : ""}
                                </p>
                            )}
                        </div>
...
export default function RAGChatbot() {
        ...
        const fetchStudentCourses = async () => {
            ...
            try {
                const res = await api.get(
                    `/institutions/slug/${inst.slug}/courses`,
                );
                const raw = Array.isArray(res?.data?.courses)
                    ? res.data.courses
                    : [];
        ...
        const fetchDocuments = async () => {
            ...
            if (user.role === "student") {
                const selectedCourseObj = courses.find(
                    (c) => String(c._id) === String(selectedCourse),
                );
                ...
                try {
                    setDocsLoading(true);
                    const res = await api.get(
                        `/institutions/slug/${inst.slug}/courses/${selectedCourseObj.code}`,
                    );
                    const list = Array.isArray(res?.data?.documents)
                        ? res.data.documents
                        : [];
    ...
    useEffect(() => {
        if (!chatScrollRef.current || (!loading && messages.length === 0))
            return;
    ...
    const handleSend = async () => {
        ...
        if (!selectedCourse) {
            setMessages((prev) => [
                ...prev,
                {
                    ...
                    type: "error",
                    content:
                        "Please select a course first so I can answer from that course documents.",
                    timestamp: new Date(),
                },
            ]);
            return;
        ...
        if (documents.length > 0 && selectedDocIds.length === 0) {
            setMessages((prev) => [
                ...prev,
                {
                    ...
                    type: "error",
                    content:
                        "Please select at least one document from the Available Docs panel.",
                    timestamp: new Date(),
                },
            ]);
            return;
        ...
        try {
            ...
            const searchMethod =
                res.search_method || res.searchMethod || "rag";
            const usedWebSearch =
                res.used_web_search ??
                res.usedWebSearch ??
                String(searchMethod).includes("web");
            const normalizedSources = normalizeSourcesForDisplay(res.sources);
                ...
                >
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-gray-500 mt-1 font-medium">
                                RAG-powered answers from your notes and web
                                context
                            </p>
                        </div>
                    </div>
                    ...
                    >
                        ...
                        <span className="hidden sm:inline">
                            {showUpload ? "Hide Docs" : "Available Docs"}
                        </span>
                    </button>
                </Motion.div>
            </div>
            ...
            <div className="flex-1 flex flex-col lg:flex-row gap-6 md:gap-8 max-w-7xl mx-auto w-full">
                ...
                >
                    ...
                    <div
                        className="h-[3px]"
                        style={{
                            background: `linear-gradient(90deg,${brand},rgba(${rgb},0.25))`,
                        }}
                    />
                    ...
                    >
                        ...
                        {messages.length === 0 && (
                            <div className="flex flex-col items-center justify-center h-full text-center pt-8 pb-4">
                                ...
                                <h3 className="text-[15px] font-extrabold text-gray-800 mb-1">
                                    Ask me anything
                                </h3>
                                <p className="text-[12px] text-gray-400 mb-6">
                                    I search your uploaded documents and the web
                                    to find answers.
                                </p>
                                {/* Suggestions */}
                                ...
                                <div className="flex items-center gap-2.5 mb-4">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-[10px] text-gray-400">
                                            Used for course-specific answers
                                        </p>
                                    </div>
                                </div>
