...
import { FiPlus, FiEdit2, FiTrash2, FiSend, FiBell } from 'react-icons/fi';
import toast from 'react-hot-toast';
import { useAuth } from '../../../context/authcontext';

export default function AnnouncementsView({ institution }) {
  const { user } = useAuth();
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            {canPost ? 'Communicate with students and teachers' : 'Stay updated with latest announcements'}
          </p>
        </div>
        {canPost && (
          <button
        ...
        {announcements.length > 0 ? (
              ...
              <div className="flex items-start justify-between mb-3">
                <div className="flex-1">
                  ...
                  <p className="text-sm text-gray-500">
                    {new Date(announcement.date).toLocaleDateString()} • {announcement.author} ({announcement.authorRole}) • {announcement.targetAudience}
                  </p>
                </div>
                {canPost && (
          ...
          <div className="text-center py-16 bg-white border border-gray-200 rounded-xl">
            <div className="w-16 h-16 bg-emerald-50 rounded-full flex items-center justify-center mx-auto mb-4">
              <FiBell className="w-8 h-8 text-emerald-400" />
            </div>
            <p className="text-gray-500 text-lg font-medium mb-2">No announcements yet</p>
            <p className="text-gray-400 text-sm">
              {canPost ? 'Create your first announcement to communicate with your institution' : 'Check back later for updates'}
            </p>
          </div>
        )}
      </div>
