...
export const createInvitation = async (req, res) => {
  try {
    ...
    if (
      ...
      (!department || !semester || !courses || courses.length === 0)
    ) {
      return res.status(400).json({
        message:
...
export const acceptInvitation = async (req, res) => {
  try {
    ...
    if (
      ...
      const UserModel =
        invitation.recipientType === "Student" ? Student : Teacher;
    ...
    if (invitation.recipientType === "Student") {
      ...
      if (student.institution) {
        return res
          .status(400)
          .json({ message: "Already joined an institution" });
      }
    ...
    // In invitationController.js - acceptInvitation function
    ...
    if (invitation.recipientType === "Teacher") {
      ...
      teacher.authorizedCourses = invitation.courses || []; // ✅ THIS IS CRITICAL
      teacher.status = "active";

      await teacher.save();

      console.log("✅ Teacher courses assigned:", teacher.authorizedCourses);
    }
...
export const deleteInvitation = async (req, res) => {
  try {
    ...
    if (
      !admin ||
      admin.institution.toString() !== invitation.institution._id.toString()
    ) {
      return res.status(403).json({ message: "Not authorized" });
    }
...
export const bulkInviteUsers = async (req, res) => {
  try {
    const { institutionId, recipientType, users, department, semester } =
      req.body;
    ...
    if (!admin || admin.institution.toString() !== institutionId) {
      return res
        .status(403)
        .json({ message: "Unauthorized institution access" });
    }
    ...
    for (let i = 0; i < users.length; i++) {
      ...
      try {
        ...
        if (existingUser?._id) {
          console.log(
            "🔔 Bulk: Emitting new invitation to user:",
            existingUser._id
          );
          global.io?.to(`user:${existingUser._id}`).emit("invitation:new", {
            _id: invite._id,
